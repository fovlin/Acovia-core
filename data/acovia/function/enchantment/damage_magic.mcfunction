execute at @s anchored eyes run summon splash_potion ^ ^ ^1 {NoGravity:true,Tags:["Acovia"],Item:{id:"minecraft:ender_pearl",components:{potion_contents:{custom_color:11534335,custom_name:"Magic",custom_effects:[{show_particles:true,amplifier:1,id:"minecraft:instant_damage"}]}}}}
...
data modify entity @n[type=splash_potion,tag=Acovia,limit=1,sort=nearest] Owner set from entity @s[type=player] UUID
